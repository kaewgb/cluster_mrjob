...
class ASPModule(object):
    ...
    def __init__(self, specializer="default_specializer", cache_dir=None, use_cuda=False, use_cilk=False, use_mapreduce=False):
        ...
        if use_cuda:
            ...
            self.backends['cuda'].toolchain.cflags.extend(["-Xcompiler","-fPIC"])
            self.backends["c++"].toolchain.cflags += ["-fPIC"]
        if use_cilk:
